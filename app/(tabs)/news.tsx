...
import { fetchNews, fetchAnnouncements, News } from '../../services/newsService';
...
export default function NewsTab() {
  ...
  // Haberleri yükle
  const loadNews = async (page = 0, sportId = selectedSportId, shouldRefresh = false) => {
    try {
      if (shouldRefresh) {
        setRefreshing(true);
      } else if (!refreshing) {
      ...
      if (isAnnouncementsTab) {
        ...
        if (response.success) {
          console.log(`Loaded ${response.data.length} news items`);
      ...
      setError(`${activeTab === 'news' ? 'Haberler' : 'Duyurular'} yüklenirken bir hata oluştu.`);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleLoadMore = () => {
    ...
    if (activeTab === 'news' && currentPage < totalPages - 1 && !loading && !refreshing) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Text style={[styles.tabButtonText, activeTab === 'news' && styles.activeTabText]}>
            Haberler
          </Text>
        </TouchableOpacity>
        <TouchableOpacity 
        ...
        >
          <Text style={[styles.tabButtonText, activeTab === 'announcements' && styles.activeTabText]}>
            Duyurular
          </Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.categoriesWrapper}>
          ...
          >
            {sportCategories.map((category, index) => (
              <View key={category.id?.toString() || 'all'} style={[
                ...
                <TouchableOpacity
                  style={[
                    ...
                    selectedSportId === category.id && styles.selectedCategoryButton,
                    index === 0 && selectedSportId === category.id && { backgroundColor: '#4cd964' }
                  ]}
                ...
                >
                  <Text 
                    style={[
                      ...
                      selectedSportId === category.id && styles.selectedCategoryText
                    ]}
                  >
                    {category.name}
                  </Text>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>
                {activeTab === 'news' ? 'Hiç haber bulunamadı.' : 'Hiç duyuru bulunamadı.'}
              </Text>
            </View>
          }
        />
